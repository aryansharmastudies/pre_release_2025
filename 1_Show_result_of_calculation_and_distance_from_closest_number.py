'''2+3 would show result: 5, 4 from closet number (9)'''
...
def CNTT(UserInput, Targets): # Closest Number To Target
        UserInputInRPN = ConvertToRPN(UserInput)
        # print(f'User Input In RPN{UserInputInRPN}')
        UserInputEvaluation = EvaluateRPN(UserInputInRPN)
        # print(f'User Input Evaluated {UserInputEvaluation}')
        difference = 999
        ClosestNumberToTarget = 0
        if UserInputEvaluation != -1:
            for Target in Targets:
                if int(Target) != -1 and int(Target) != UserInputEvaluation:
                    #print(f'EVALUATING if {Target} > {UserInputEvaluation}')
                    if int(Target) > UserInputEvaluation:
                        calc_difference = int(Target) - UserInputEvaluation
                    else: 
                        calc_difference = UserInputEvaluation - int(Target)
                    #print(f'DIFFERENCE = {calc_difference}')
                    if calc_difference < difference:
                        difference = calc_difference
                        ClosestNumberToTarget = int(Target)
            print(f'(づ￣ 3￣)づ {UserInput} would show result {UserInputEvaluation}, {difference} from closest number ({ClosestNumberToTarget})')
            print()
        else:
            print(f'RPN evaluated to -1 つ﹏⊂')
            print()
