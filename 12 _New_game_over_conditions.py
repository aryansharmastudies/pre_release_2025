'''
##12 -  New game over conditions
Change the game so that it only ends when the user goes into a negative score. 
If the target numbers get to the left side of the list (normal game over condition), 
then -1 is taken from the score and that number is removed.

`|17|40|8|17|` in this scenario, 17 would move off the screen and the game would be over. 
Instead, 17 moves off the screen, -1 from the score and move everything left e.g. `|40|8|17|`.
...
def Main():
    ...
    if Choice == "y":
        ...
        MaxTarget = 1000
        TrainingGame = True
        Targets = [-1, -1, -1, -1, -1, 24, 9, 140, 16, 121, 9, 45, 16, 75, 9, 23, 119, 43, 24, 119]
    else:
        MaxNumber = 10
        MaxTarget = 50
