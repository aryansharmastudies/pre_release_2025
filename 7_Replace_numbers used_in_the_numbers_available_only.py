...
def RemoveNumbersUsed(UserInput, MaxNumber, NumbersAllowed):
    UserInputInRPN = ConvertToRPN(UserInput)
    copy_of_User_input_in_RPN.clear()
    print(f'{copy_of_User_input_in_RPN}')
    for Item in UserInputInRPN:
        if CheckValidNumber(Item, MaxNumber):
            if int(Item) in NumbersAllowed:
                copy_of_User_input_in_RPN.append(int(Item))
...
def GetNumber(MaxNumber):
    new_numba = random.randint(1, MaxNumber)
    print(f'GYA + {copy_of_User_input_in_RPN}')
    while new_numba in copy_of_User_input_in_RPN:
        new_numba = random.randint(1, MaxNumber)
    return new_numba   
